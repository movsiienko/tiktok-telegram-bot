 ...
 * Protected by SETUP_SECRET bearer token.
 *
 * Body options:
 *   { "action": "webhook_info" }            → getWebhookInfo
 *   { "action": "test", "url": "...", "chat_id": 123 } → full pipeline test
 */
...
export async function POST(request: Request): Promise<Response> {
  ...
  const body = await request.json() as { action?: string; url?: string; chat_id?: string | number };
  ...
  if (!url || chat_id === undefined || chat_id === null) {
    return Response.json({ error: "Need url and chat_id (or action)" }, { status: 400 });
  }

  const steps: Record<string, unknown> = {};

  try {
    const apiUrl = `https://www.tikwm.com/api/?url=${encodeURIComponent(String(url))}&hd=1`;
    ...
    steps.videoUrl = videoUrl.slice(0, 80) + "…";

    const videoRes = await fetch(videoUrl, {
      headers: { "User-Agent": "Mozilla/5.0", Referer: "https://www.tiktok.com/" }
    });
    steps.videoHttpStatus = videoRes.status;
    ...
    const sendJson = await sendRes.json() as { ok?: boolean; description?: string; result?: unknown };
    steps.sendVideoStatus = sendRes.status;
    steps.sendVideoOk = sendJson.ok;
    steps.sendVideoDesc = sendJson.description;
